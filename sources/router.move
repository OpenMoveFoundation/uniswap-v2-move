module uniswap_v2::router {
    ...
    fun calc_optimal_coin_values(
        ...
        if (!utils::is_sorted_two(token_a, token_b)) {
        ...
        if (reserve_a == 0 && reserve_b == 0) {
            ...
            if (amount_b_optimal <= amount_b_desired) {
                ...
                let amount_a_optimal = utils::quote(
                    amount_b_desired, reserve_b, reserve_a
                );
    ...
    inline fun validate_path_start(
        path: &vector<address>, expected: address
    ) {
    ...
    inline fun ensure_sufficient_balance<CoinType>(
        ...
        if (balance < required_amount) {
            ...
            if (is_apt) {
                wrap_apt(sender, amount_to_deposit);
            } else {
    ...
    */
    public entry fun wrap_coin<CoinType>(sender: &signer, amount: u64) {
    ...
    public entry fun wrap_apt(sender: &signer, amount: u64) {
        let to = signer::address_of(sender);
    ...
    fun add_liquidity_internal(
        ...
        let (token0, token1) = utils::sort_two_tokens(tokenA, tokenB);
        let (amount0, amount1) =
            if (token0 == tokenA) {
        ...
        let (amount0Optimal, amount1Optimal) =
    ...
    public entry fun add_liquidity_apt(
        sender: &signer,
        token: address,
        amount_token_desired: u64,
        amount_token_min: u64,
        amount_apt_desired: u64,
        amount_apt_min: u64,
        to: address,
        deadline: u64
    ) {
        validate_token_pair(token, FA_APT);
        validate_amount(amount_token_desired);
        validate_amount(amount_apt_desired);
        ensure(deadline);

        let sender_addr = signer::address_of(sender);
        let token_object = object::address_to_object<Metadata>(token);
        let apt_object = option::destroy_some(coin::paired_metadata<AptosCoin>());
        let apt_addr = object::object_address(&apt_object);

        let (token0, token1) = utils::sort_two_tokens(token_object, apt_object);

        if (!factory::pair_exists(token0, token1)) {
            factory::create_pair(sender, token, apt_addr);
        };

        // Calculate amounts based on token order
        let (amount0_desired, amount1_desired) =
            if (token0 == token_object) {
                (amount_token_desired, amount_apt_desired)
            } else {
                (amount_apt_desired, amount_token_desired)
            };

        let (amount0_min, amount1_min) =
            if (token0 == token_object) {
                (amount_token_min, amount_apt_min)
            } else {
                (amount_apt_min, amount_token_min)
            };
        ...
        let apt_object_balance = primary_fungible_store::balance(
            sender_addr, apt_object
        );
        if (apt_object_balance
            < (if (token0 == apt_object) { amount0 }
            else { amount1 })) {
            let amount_apt_to_deposit =
                (if (token0 == apt_object) {
                    amount0
                } else {
                    amount1
                }) - apt_object_balance;
            wrap_apt(sender, amount_apt_to_deposit);
        };
        ...
        let (asset0, asset1) =
            if (token0 == token_object) {
                (
                    ...
                    primary_fungible_store::withdraw(sender, apt_object, amount1)
                )
            } else {
                (
                    primary_fungible_store::withdraw(sender, apt_object, amount0),
    ...
    public entry fun add_liquidity_coin<CoinType>(
        ...
        let (token0, token1) = utils::sort_two_tokens(token_object, coin_object);
        ...
        let coin_object_balance =
            primary_fungible_store::balance(sender_addr, coin_object);
        if (coin_object_balance
            < (if (token0 == coin_object) { amount0 }
            else { amount1 })) {
            let amount_coin_to_deposit =
                (if (token0 == coin_object) {
                    amount0
                } else {
                    amount1
                }) - coin_object_balance;
    ...
    fun remove_liquidity_internal(
        ...
        let expected_a = (liquidity as u128) * (reserve_a as u128)
            / (total_supply as u128);
        let expected_b = (liquidity as u128) * (reserve_b as u128)
            / (total_supply as u128);
        ...
        if (utils::is_sorted_two(tokenA, tokenB)) {
            (redeemedA, redeemedB)
        } else {
            (redeemedB, redeemedA)
        }
    ...
    public entry fun remove_liquidity_apt(
        sender: &signer,
        token: address,
        liquidity: u64,
        amount_token_min: u64,
        ...
        primary_fungible_store::deposit(to, amount_apt);
    }
    ...
    public fun swap(
        ...
        let (token0, token1) = utils::sort_two_tokens(from_token, to_token);
        ...
        let (zero, coins_out);
        if (utils::is_sorted_two(from_token, to_token)) {
            (zero, coins_out) = pair::swap(
                sender,
                pair,
                token_in,
                0,
                fungible_asset::zero(to_token),
                amount_out,
                to
            );
        } else {
            (coins_out, zero) = pair::swap(
                sender,
                pair,
                fungible_asset::zero(to_token),
                amount_out,
    ...
    public entry fun swap_exact_tokens_for_tokens(
        ...
        if (length == 2) {
            assert!(
                current_amount_in >= amount_out_min, ERROR_INSUFFICIENT_OUTPUT_AMOUNT
            );
        };
        ...
        while (i < length - 1) {
            ...
            let to_token =
                object::address_to_object<Metadata>(*vector::borrow(&path, i + 1));
            let in =
                primary_fungible_store::withdraw(sender, from_token, current_amount_in);
            let out = swap(sender, in, to_token, to);
    ...
    public entry fun swap_tokens_for_exact_tokens(
        ...
        while (i > 0) {
            let from_token =
                object::address_to_object<Metadata>(*vector::borrow(&path, i - 1));
            let to_token = object::address_to_object<Metadata>(
            ...
            let amount_in =
                utils::get_amount_in(current_amount_out, reserve_in, reserve_out);
    ...
    public entry fun swap_exact_apt_for_tokens(
        sender: &signer,
        amount_apt: u64,
        amount_out_min: u64,
        path: vector<address>,
        ...
        assert!(*vector::borrow(&path, 0) == FA_APT, ERROR_INVALID_PATH);

        let sender_addr = signer::address_of(sender);
        let length = vector::length(&path);
        let current_amount_in = amount_apt;

        // Handle APT wrapping only for the first swap
        let apt_object = option::destroy_some(coin::paired_metadata<AptosCoin>());
        let apt_object_balance = primary_fungible_store::balance(
            sender_addr, apt_object
        );
        if (apt_object_balance < amount_apt) {
            let amount_apt_to_deposit = amount_apt - apt_object_balance;
            wrap_apt(sender, amount_apt_to_deposit);
        };

        // First swap (APT to first token)
        let to_token = object::address_to_object<Metadata>(*vector::borrow(&path, 1));
        let in = primary_fungible_store::withdraw(sender, apt_object, current_amount_in);
        let out = swap(sender, in, to_token, to);
        current_amount_in = fungible_asset::amount(&out);
    ...
    public entry fun swap_tokens_for_exact_apt(
        sender: &signer,
        amount_out: u64,
        amount_in_max: u64,
        path: vector<address>,
        ...
        assert!(
            *vector::borrow(&path, vector::length(&path) - 1) == FA_APT,
            ERROR_INVALID_PATH
        );
    ...
    public entry fun swap_exact_tokens_for_apt(
        sender: &signer,
        amount_in: u64,
        amount_out_min: u64,
        path: vector<address>,
    ...
    public entry fun swap_apt_for_exact_tokens(
        sender: &signer,
        amount_apt_max: u64,
        amount_out: u64,
        path: vector<address>,
        ...
        // Check if first amount (APT amount) is within limits
        let apt_amount = *vector::borrow(&amounts, vector::length(&amounts) - 1);
        assert!(apt_amount <= amount_apt_max, ERROR_INSUFFICIENT_INPUT_AMOUNT);
        ...
        if (apt_object_balance < apt_amount) {
            let amount_apt_to_deposit = apt_amount - apt_object_balance;
            wrap_apt(sender, amount_apt_to_deposit);
        };
    ...
    */
    ...
    public fun get_amounts_out(
        amount_in: u64, path: vector<Object<Metadata>>
    ): vector<u64> {
        ...
        *vector::borrow_mut(&mut amounts, 0) = amount_in;
        ...
        while (k < path_length - 1) {
            ...
            let (token0, token1) = utils::sort_two_tokens(token_a, token_b);
            let pair = pair::liquidity_pool(token0, token1);
            let (reserve0, reserve1, _) = pair::get_reserves(pair);
    ...
    public fun get_amounts_in(
        amount_out: u64, path: vector<Object<Metadata>>
    ): vector<u64> {
        assert!(vector::length(&path) >= 2, ERROR_INVALID_PATH);
        let amounts = vector::empty<u64>();
        let path_length = vector::length(&path);
