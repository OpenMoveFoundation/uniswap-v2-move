module uniswap_v2::factory {
    ...
    public fun is_initialized(): bool {
        exists<Factory>(@uniswap_v2)
    }

    public entry fun create_pair(
        sender: &signer, tokenA: address, tokenB: address
    ) acquires Factory {
        ...
        let (token0, token1) = utils::sort_two_tokens(token0_object, token1_object);
    ...
    public fun all_pairs_paginated(start: u64, limit: u64): vector<address> acquires Factory {
        ...
        let len = smart_vector::length(all_pairs);
        let end = if (start + limit > len) { len }
        else {
            start + limit
        };
    ...
    #[view]
    public fun get_reserves(token_a: address, token_b: address): (u64, u64) {
        ...
        let (token0, token1) = utils::sort_two_tokens(
            token_a_metadata, token_b_metadata
        );
    ...
    public fun pair_exists(
        tokenA: Object<Metadata>, tokenB: Object<Metadata>
