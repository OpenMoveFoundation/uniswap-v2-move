module uniswap_v2::pair {
    ...
    public fun balance_of(pair: Object<Pair>, token: Object<Metadata>): u64 acquires Pair {
        ...
        if (object::object_address(&token) == object::object_address(&pair_data.token0)) {
            fungible_asset::balance(pair_data.token0)
        } else if (object::object_address(&token)
            == object::object_address(&pair_data.token1)) {
            fungible_asset::balance(pair_data.token1)
        } else { 0 }
    }
    ...
    public(friend) fun initialize(
        ...
        if (!utils::is_sorted_two(token0, token1)) {
        ...
        let lp_token =
            object::object_from_constructor_ref<Metadata>(pair_constructor_ref);
    ...
    inline fun update(
        ...
        if (time_elapsed > 0 && reserve0 != 0 && reserve1 != 0) {
            // allow overflow u128
            let price_0_cumulative_last_delta =
                fixedpoint64::to_u128(fixedpoint64::fraction(reserve1, reserve0))
                    * time_elapsed;
            lp.price_0_cumulative_last = math::overflow_add(
                lp.price_0_cumulative_last, price_0_cumulative_last_delta
            );

            let price_1_cumulative_last_delta =
                fixedpoint64::to_u128(fixedpoint64::fraction(reserve0, reserve1))
                    * time_elapsed;
            lp.price_1_cumulative_last = math::overflow_add(
                lp.price_1_cumulative_last, price_1_cumulative_last_delta
            );
        };
    ...
    inline fun mint_fee(pair: Object<Pair>): bool acquires Pair {
        let lp_address = object::object_address(&pair);
        let lp = pair_data_mut(&pair);
        let fee_on = owner::get_fee_on();
        let fee_to = owner::get_fee_to();
            ...
            if (k_last != 0) {
                ...
                if (root_k > root_k_last) {
                    ...
                    if (math::is_overflow_mul(total_supply, delta_k)) {
                        ...
                        let denominator = (root_k as u256) * (20)
                            + (root_k_last as u256);
                        ...
                        let denominator = (root_k as u128) * (20)
                            + (root_k_last as u128);
                        let liquidity = ((numerator / denominator) as u64);
                        if (liquidity > 0) {
                            mint_internal(pair, fee_to, liquidity);
    ...
    inline fun mint_internal(
        pair: Object<Pair>, to: address, amount: u64
    ) acquires Pair {
        let pair_data = pair_data(&pair);
        let acc_store = utils::ensure_account_token_store(to, pair);
    ...
    public(friend) fun mint(
        ...
        let acc_store = utils::ensure_account_token_store(to, pool);
        ...
        if (total_supply == 0) {
            assert!(
                ...
                ERROR_INSUFFICIENT_LIQUIDITY_MINT
            );
            liquidity = math::sqrt(amount0, amount1) - MINIMUM_LIQUIDITY;
            primary_fungible_store::mint(mint_ref, @uniswap_v2, MINIMUM_LIQUIDITY);
        } else {
        ...
        fungible_asset::deposit_with_ref(
            &lp.lp_token_refs.transfer_ref, acc_store, lp_coins
        );
    ...
    public(friend) fun burn(
        sender: &signer, pair: Object<Pair>, amount: u64
    ): (FungibleAsset, FungibleAsset) acquires Pair {
        assert_locked(pair);
        owner::assert_unpaused();
        assert!(amount > 0, ERROR_ZERO_AMOUNT);
        let sender_addr = signer::address_of(sender);
        let store = utils::ensure_account_token_store(sender_addr, pair);
        ...
        assert!(
            amount0 > 0 && amount1 > 0,
            ERROR_INSUFFICIENT_LIQUIDITY_BURN
        );

        let swap_signer = &owner::get_signer();

        fungible_asset::burn_from(&lp.lp_token_refs.burn_ref, store, amount);

        let redeemed0 = dispatchable_fungible_asset::withdraw(
            swap_signer, store0, amount0
        );
        let redeemed1 = dispatchable_fungible_asset::withdraw(
            swap_signer, store1, amount1
        );
        ...
        if (utils::is_sorted_two(
            fungible_asset::store_metadata(lp.token0),
            fungible_asset::store_metadata(lp.token1)
        )) {
            (redeemed0, redeemed1)
        } else {
            (redeemed1, redeemed0)
    ...
    public(friend) fun swap(
        ...
        assert!(
            ...
            ERROR_INSUFFICIENT_INPUT_AMOUNT
        );
        assert!(
        ...
        let token0_out =
            dispatchable_fungible_asset::withdraw(swap_signer, store0, amount0_out);
        let token1_out =
            dispatchable_fungible_asset::withdraw(swap_signer, store1, amount1_out);
    ...
    inline fun assert_k_increase(
        ...
        let scale = 100000000;
        if (math::is_overflow_mul(balance0_adjusted, balance1_adjusted)
            || math::is_overflow_mul(balance01_old_not_scaled, scale)) {
            let balance01_adjusted =
                ((balance0_adjusted as u256) * (balance1_adjusted as u256));
            ...
            assert!(
                balance0_adjusted * balance1_adjusted
                    >= balance01_old_not_scaled * scale,
                ERROR_K_ERROR
            );
        };
    }
    ...
    public fun liquidity_pool_address(
        ...
        object::create_object_address(&owner::get_signer_address(), seed)
    }

    inline fun pair_data<T: key>(pair: &Object<T>): &Pair acquires Pair {
    ...
    inline fun create_lp_token(
        ...
        let seed = get_pair_seed(token0, token1);
        let lp_token_constructor_ref =
            &object::create_named_object(&owner::get_signer(), seed);
        primary_fungible_store::create_primary_store_enabled_fungible_asset(
            lp_token_constructor_ref,
            option::none(),
            token_name,
            string::utf8(b"UNI V2 LP"),
            LP_TOKEN_DECIMALS,
            string::utf8(b"https://youripfs.com/lp-image.png"),
            string::utf8(b"https://yourwebsite.com")
        );
    ...
    inline fun lp_token_name(
        ...
        let token_symbol = string::utf8(b"Uniswap ");
    ...
    public inline fun get_pair_seed(
        token0: Object<Metadata>, token1: Object<Metadata>
    ): vector<u8> {
        let (tokenA, tokenB) = utils::sort_two_tokens(token0, token1);
        let seeds = vector[];
        seeds.append(bcs::to_bytes(&object::object_address(&tokenA)));
        seeds.append(bcs::to_bytes(&object::object_address(&tokenB)));
        seeds
    }

    inline fun create_token_store(
        pair_signer: &signer, token: Object<Metadata>
