...
module uniswap_v2::pair_tests {
    ...
    fun test_mint(admin: &signer, deployer: &signer, bob: &signer) {
        ...
        let (tokenA, tokenB) = utils::sort_two_tokens(usdt_metadata, usdc_metadata);

        factory::create_pair(
            bob, object::object_address(&tokenA), object::object_address(&tokenB)
        );
        ...
        let token0_to_withdraw =
            primary_fungible_store::withdraw(bob, tokenA, token0_amount);
        let token1_to_withdraw =
            primary_fungible_store::withdraw(bob, tokenB, token1_amount);
        ...
        assert!(
            bob_lp_balance == (expected_liquidity - MINIMUM_LIQUIDITY),
            1
        );
        assert!(pool_balance0 == token0_amount, 2);
    ...
    #[test(admin = @admin, deployer = @uniswap_v2, bob = @bob)]
    fun test_burn(admin: &signer, deployer: &signer, bob: &signer) {
        account::create_account_for_test(signer::address_of(bob));
        owner_tests::setup_test_with_genesis(deployer);
        test_coins::init_coins();
        ...
        let (tokenA_to_withdraw, tokenB_to_withdraw) =
            pair::burn(bob, created_pair, expected_liquidity - MINIMUM_LIQUIDITY);
        ...
        let total_supply_token0 = usdt_token::total_supply();
        let total_supply_token1 = usdc_token::total_supply();

        let bob_token0_balance = usdt_token::balance_of(@bob);
        ...
        assert!(
            bob_token0_balance == ((total_supply_token0 as u64) - 1000),
            4
        );
        assert!(
            bob_token1_balance == ((total_supply_token1 as u64) - 1000),
            5
        );
    }

    #[test(admin = @admin, deployer = @uniswap_v2, bob = @bob)]
    fun test_swap(admin: &signer, deployer: &signer, bob: &signer) {
        account::create_account_for_test(signer::address_of(bob));
        owner_tests::setup_test_with_genesis(deployer);
        test_coins::init_coins();
        ...
        let usdt_metadata = usdt_token::metadata();
        let usdc_metadata = usdc_token::metadata();
        let (token0, token1) = utils::sort_two_tokens(usdt_metadata, usdc_metadata);

        // Create pair and add initial liquidity
        factory::create_pair(
            bob, object::object_address(&token0), object::object_address(&token1)
        );
    ...
    #[test(admin = @admin, deployer = @uniswap_v2, bob = @bob)]
    fun test_mint_fee(admin: &signer, deployer: &signer, bob: &signer) {
        account::create_account_for_test(signer::address_of(bob));
        owner_tests::setup_test_with_genesis(deployer);
        test_coins::init_coins();
        ...
        // Enable protocol fee
        owner::set_fee_to(deployer, @admin);
        ...
        let token0_deposit2 = primary_fungible_store::withdraw(
            bob, token0, token0_amount
        );
        let token1_deposit2 = primary_fungible_store::withdraw(
            bob, token1, token1_amount
        );
    ...
    #[test(admin = @admin, deployer = @uniswap_v2, bob = @bob)]
    #[expected_failure(abort_code = 5, location = uniswap_v2::pair)]
    ...
    fun test_swap_edge_cases(
        ...
        let (token0_out, token1_out) =
            pair::swap(bob, pair, empty_token0, 0, empty_token1, 0, @bob);
    ...
    fun test_swap_large_amounts(
        ...
        test_coins::mint_usdt(admin, signer::address_of(bob), large_amount);
    ...
    #[test(admin = @admin, deployer = @uniswap_v2, bob = @bob)]
    #[expected_failure(abort_code = 22, location = uniswap_v2::pair)]
    ...
    fun test_mint_zero_supply(
        ...
        pair::mint(bob, token0_small, token1_small, @bob);
    }
    ...
    fun test_burn_insufficient_liquidity(
        ...
        let (token0_out, token1_out) = pair::burn(bob, pair, 1000 * pow(10, 8));
    ...
    fun test_cumulative_prices(
        ...
        let (price0_initial, price1_initial) = pair::get_cumulative_prices(pair);
        ...
        let (price0_after, price1_after) = pair::get_cumulative_prices(pair);
        assert!(price0_after >= price0_initial, 0);
        assert!(price1_after >= price1_initial, 1);

        // Test individual price getters
        let price0_last = pair::price_0_cumulative_last(pair);
        let price1_last = pair::price_1_cumulative_last(pair);
