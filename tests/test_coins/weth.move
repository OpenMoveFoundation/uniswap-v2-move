...
module uniswap_v2::weth_token {
    ...
    const ASSET_SYMBOL: vector<u8> = b"WETH";
    const ASSET_NAME: vector<u8> = b"Wrapped Ether";
    const DECIMALS: u8 = 8;
    ...
    fun owner(): address acquires Ownership {
        let owner = borrow_global<Ownership>(weth_token_address()).owner;
        return owner
    }

    #[view]
    public fun weth_token_address(): address {
        object::create_object_address(&@uniswap_v2, ASSET_SYMBOL)
    }

    #[view]
    public fun metadata(): Object<Metadata> {
        object::address_to_object(weth_token_address())
    }
    ...
    fun init_module(admin: &signer) {
        ...
        primary_fungible_store::create_primary_store_enabled_fungible_asset(
            ...
            utf8(ASSET_SYMBOL),
            DECIMALS,
            utf8(b"https://youripfs.com/lp-image.png"),
            utf8(b"https://yourwebsite.com")
        );

        fungible_asset::set_untransferable(constructor_ref);

        let metadata_object_signer = &object::generate_signer(constructor_ref);
        move_to(metadata_object_signer, Ownership { owner: ADMIN_ADDRESS });
    ...
    public entry fun mint(minter: &signer, to: address, amount: u64) acquires Management, Ownership, State {
        ...
        let management = borrow_global<Management>(weth_token_address());
    ...
    public entry fun burn(
        minter: &signer, from: address, amount: u64
    ...
    public entry fun set_pause(admin: &signer, paused: bool) acquires Ownership, State {
        let ownership = borrow_global<Ownership>(weth_token_address());
        assert!(signer::address_of(admin) == ownership.owner, ERROR_UNAUTHORIZED);
        let state = borrow_global_mut<State>(weth_token_address());
    ...
    public entry fun denylist(admin: &signer, account: address) acquires Management, Ownership, State {
        ...
        let freeze_ref = &borrow_global<Management>(weth_token_address()).transfer_ref;
    ...
    public entry fun transfer_ownership(
        admin: &signer, new_owner: address
    ) acquires Ownership, State {
        assert_not_paused();
        let ownership = borrow_global_mut<Ownership>(weth_token_address());
        assert!(signer::address_of(admin) == ownership.owner, ERROR_UNAUTHORIZED);
    ...
    fun assert_not_paused() acquires State {
        let state = borrow_global<State>(weth_token_address());
        ...
        if (primary_fungible_store::primary_store_exists_inlined(account, metadata)) {
            assert!(
                !fungible_asset::is_frozen(
                    primary_fungible_store::primary_store_inlined(account, metadata)
                ),
                ERROR_DENYLISTED
            );
        }
