...
module uniswap_v2::wbtc_token {
    ...
    fun owner(): address acquires Ownership {
        let owner = borrow_global<Ownership>(wbtc_token_address()).owner;
        return owner
    }

    #[view]
    public fun wbtc_token_address(): address {
        object::create_object_address(&@uniswap_v2, ASSET_SYMBOL)
    }

    #[view]
    public fun metadata(): Object<Metadata> {
        object::address_to_object(wbtc_token_address())
    }
    ...
    public entry fun mint(minter: &signer, to: address, amount: u64) acquires Management, Ownership, State {
        ...
        let management = borrow_global<Management>(wbtc_token_address());
    ...
    public entry fun set_pause(admin: &signer, paused: bool) acquires Ownership, State {
        let ownership = borrow_global<Ownership>(wbtc_token_address());
        assert!(signer::address_of(admin) == ownership.owner, ERROR_UNAUTHORIZED);
        let state = borrow_global_mut<State>(wbtc_token_address());
    ...
    public entry fun transfer_ownership(
        ...
        assert_not_paused();
        let ownership = borrow_global_mut<Ownership>(wbtc_token_address());
        assert!(signer::address_of(admin) == ownership.owner, ERROR_UNAUTHORIZED);
    ...
    fun assert_not_paused() acquires State {
        let state = borrow_global<State>(wbtc_token_address());
