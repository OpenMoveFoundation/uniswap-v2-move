#[test_only]
module uniswap_v2::router_tests {
  use std::signer;
  use std::vector;
  ...
  #[test(admin = @admin, deployer = @uniswap_v2, bob = @bob, alice = @alice)]
  fun test_add_liquidity(
    ...
    test_coins::mint_wbtc(admin, signer::address_of(bob), 100 * pow(10, 8));
    test_coins::mint_usdc(admin, signer::address_of(bob), 100 * pow(10, 8));
    test_coins::mint_wbtc(admin, signer::address_of(alice), 100 * pow(10, 8));
    ...
    let wbtc_metadata = wbtc_token::metadata();
    let usdc_metadata = usdc_token::metadata();

    let (token0, token1) = utils::sort_two_tokens(wbtc_metadata, usdc_metadata);
    ...
    let pool_lp_balance = pair::lp_balance_of(@uniswap_v2, pair);
  ...
  fun test_add_liquidity_with_less_x_ratio(
    ...
    let wbtc_metadata = wbtc_token::metadata();
    let usdc_metadata = usdc_token::metadata();
    let wbtc_address = wbtc_token::wbtc_token_address();
    ...
    let bob_token_x_before_balance = if (token0_address == wbtc_address) wbtc_token::balance_of(@bob) else usdc_token::balance_of(@bob);
    let bob_token_y_before_balance = if (token0_address == wbtc_address) usdc_token::balance_of(@bob) else wbtc_token::balance_of(@bob);
    ...
    let bob_token_x_after_balance = if (token0_address == wbtc_address) wbtc_token::balance_of(@bob) else usdc_token::balance_of(@bob);
    let bob_token_y_after_balance = if (token0_address == wbtc_address) usdc_token::balance_of(@bob) else wbtc_token::balance_of(@bob);
    let bob_lp_balance = pair::lp_balance_of(@bob, pair);
    let deployer_lp_balance = pair::lp_balance_of(@uniswap_v2, pair);
  ...
  #[test(admin = @admin, deployer = @uniswap_v2, bob = @bob, alice = @alice)]
  #[expected_failure(abort_code = 12, location = uniswap_v2::router)]
  ...
  #[test(admin = @admin, deployer = @uniswap_v2, bob = @bob, alice = @alice)]
  #[expected_failure(abort_code = 11, location = uniswap_v2::router)]
  ...
  fun test_remove_liquidity(
    ...
    test_coins::init_coins();
    test_coins::mint_wbtc(admin, signer::address_of(bob), 100000 * pow(10, 8));
    ...
    let total_supply_token0 = if (token0_address == wbtc_address) wbtc_token::total_supply() else usdc_token::total_supply();
    let total_supply_token1 = if (token0_address == wbtc_address) usdc_token::total_supply() else wbtc_token::total_supply();
    let lp_total_supply = pair::lp_token_supply(pair);

    let bob_token0_balance = if (token0_address == wbtc_address) wbtc_token::balance_of(@bob) else usdc_token::balance_of(@bob);
    let bob_token1_balance = if (token0_address == wbtc_address) usdc_token::balance_of(@bob) else wbtc_token::balance_of(@bob);
  ...
  fun test_swap_exact_tokens_for_tokens(
    ...
    test_coins::mint_wbtc(admin, signer::address_of(bob), 1000 * pow(10, 8));
    ...
    assert!(bob_wbtc_before - bob_wbtc_after == amount_in, 1); // Exact amount in was taken
  ...
  fun test_swap_tokens_for_exact_tokens(
    ...
    assert!(bob_wbtc_before - bob_wbtc_after <= max_amount_in, 2); // Didn't spend more than max
  }

  #[test(admin = @admin, deployer = @uniswap_v2, bob = @bob)]
  #[expected_failure(abort_code = 1, location = uniswap_v2::router)]
  ...
  #[test(admin = @admin, deployer = @uniswap_v2, bob = @bob)]
  #[expected_failure(abort_code = 3, location = uniswap_v2::router)]
  ...
  fun test_multi_hop_swap(
    ...
    // Mint tokens for bob
    test_coins::mint_wbtc(admin, signer::address_of(bob), 200 * pow(10, 8));
